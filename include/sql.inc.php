...
function sql_set_websettings() {
	global $config, $mysql;
	$query = $mysql->query("SELECT * FROM `".$config->db_prefix."_webconfig`") or die ($mysql->errorInfo());
	$query->execute();
...
function sql_get_amxadmins_server($server) {
	...
	while($result = $query->fetch(PDO::FETCH_OBJ)) {
		...
		$query2->execute();
		if($result2 = $query2->fetch(PDO::FETCH_OBJ)) {
...
function sql_get_comments_count_fail($repair=0) {
	...
	if(!$repair) return $query->rowCount();
	$query->execute();
...
function sql_get_search_bans($search,$active=1,&$count=0) {
	...
	while($result = $query->fetch(PDO::FETCH_OBJ)) {
		if(!empty($result->player_id)) {
			...
			$query->execute();
			$bancount = $query2->fetch(PDO::FETCH_OBJ)[0];
		}
		$ban_row=[
			"bid" 			=> $result->bid,
